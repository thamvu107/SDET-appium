...
//    }
  ...
  private <T extends Component> By getComponentLocator(Class<T> componentClass) {
    if (componentClass.isAnnotationPresent(ComponentXpath.class)) {
      return AppiumBy.xpath(componentClass.getAnnotation(ComponentXpath.class).value());
    } else if (componentClass.isAnnotationPresent(ComponentAccessibilityId.class)) {
      return AppiumBy.accessibilityId(componentClass.getAnnotation(ComponentAccessibilityId.class).value());
    } else {
      String errorMessage = String.format("Component class %s must have annotation %s or %s",
                                          componentClass.getSimpleName(),
                                          ComponentAccessibilityId.class.getSimpleName(),
                                          ComponentXpath.class.getSimpleName());
      log.atError().setMessage(errorMessage).log();
      throw new IllegalArgumentException(errorMessage);
    }
  }
